...
<head>
    <meta charset="UTF-8">
    <title>Laporan Surat Keluar Tahun {{ $tahun }}</title>
    <style>
        body { 
        ...
        .signature-box { width: 220px; text-align: center; margin: 0 auto; }
...
<body>
    ...
    <table class="data-table">
        ...
        <tbody>
            @forelse($suratKeluars as $index => $item)
                @php
                ...
                <tr>
                    ...
                    <td>{{ $item->bidang ? $item->bidang->nama : '-' }}</td>
                    <td class="text-center">{{ $tglFormatted }}</td>
                    <td>{{ $item->perihal }}</td>
                    <td>{{ $item->tujuan ?? $item->tujuan_penerima ?? '-' }}</td>
                    <td>{{ $item->pembuat_surat ?? '-' }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="8" class="text-center" style="padding: 15px;">Tidak ada data surat keluar pada tahun {{ $tahun }}.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
