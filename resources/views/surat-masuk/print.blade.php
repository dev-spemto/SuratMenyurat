...
<head>
    ...
    @php
        ...
        // Tanggal Surat
        $rawTglSurat = $suratMasuk->tanggal_surat ?? $suratMasuk->tgl_surat ?? null;
        ...
        // Tanggal Diterima
        $rawTglTerima = $suratMasuk->tanggal_diterima ?? $suratMasuk->tanggal_terima ?? $suratMasuk->tgl_terima ?? null;
    ...
    @endphp
    <title>Arsip Surat Masuk - {{ $suratMasuk->nomor_surat }}</title>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        @media print {
            ...
            .paper { margin: 0 auto; width: 210mm; min-height: 297mm; box-shadow: none; border: none; }
            .attachment-page { page-break-before: always !important; }
        }
    </style>
</head>
...
<body>
    ...
    <div class="toolbar">
        <a href="{{ route('surat-masuk.show', $suratMasuk->id) }}" class="button button-back">← Kembali</a>
    ...
    <div class="paper">
        <div>
            ...
            {{-- JUDUL --}}
            <div class="title">
                <h2>LEMBAR REGISTRASI SURAT MASUK</h2>
                <p>Nomor Agenda / Surat: {{ $suratMasuk->nomor_surat }}</p>
            </div>

            {{-- CONTENT --}}
            <div class="content">
                <p>
                    Dokumen ini diterbitkan sebagai <strong>Bukti Registrasi dan Lembar Pengesahan Arsip Resmi</strong> untuk pencatatan riwayat surat masuk pada sistem administrasi persuratan sekolah. Rincian data administratif adalah sebagai berikut:
                </p>

                <table class="detail-table">
                    <tr>
                        <td class="detail-label">Nomor Surat Asal</td>
                    ...
                    <tr>
                        ...
                        <td class="detail-colon">:</td>
                        <td>{{ $suratMasuk->jenisSurat ? ($suratMasuk->jenisSurat->kode . ' - ' . $suratMasuk->jenisSurat->nama) : ($suratMasuk->jenis_surat ?? '-') }}</td>
                    </tr>
                    <tr>
                        <td class="detail-label">Pengirim / Instansi</td>
                        <td class="detail-colon">:</td>
                        <td>{{ $suratMasuk->pengirim ?? $suratMasuk->asal_surat ?? '-' }}</td>
                    </tr>
                    <tr>
                        <td class="detail-label">Tanggal Surat</td>
                        <td class="detail-colon">:</td>
                        <td>{{ $tglSuratF }}</td>
                    </tr>
                    <tr>
                        <td class="detail-label">Tanggal Diterima</td>
                        <td class="detail-colon">:</td>
                        <td>{{ $tglTerimaF }}</td>
                    </tr>
                    <tr>
                        <td class="detail-label">Penerima / Petugas TU</td>
                        <td class="detail-colon">:</td>
                        <td>{{ $suratMasuk->penerima ?? $suratMasuk->penerima_surat ?? '-' }}</td>
                    </tr>
                    <tr>
                ...
                <div class="body-text">
                    ...
                    @if ($suratMasuk->keterangan)
                        <p>
                            <strong>Catatan / Disposisi Tambahan:</strong><br>
                    ...
                    <tr>
                        <td>
                            <div class="signature-box-left">
                                <div style="visibility: hidden;">{{ $pengaturan?->kota ?? 'Tonjong' }}, {{ $tglTerimaF }}</div>
                        ...
                        <td>
                            <div class="signature-box-right">
                                <div>{{ $pengaturan?->kota ?? 'Tonjong' }}, {{ $tglTerimaF }}</div>
    ...
    @php
        $filePath = $suratMasuk->file_surat ?? $suratMasuk->lampiran ?? null;
    @endphp
    @if ($filePath)
        @php
            ...
            <div class="paper attachment-page">
                <div>
                    <div class="attachment-header">
                        <h3>LAMPIRAN DOKUMEN SURAT MASUK</h3>
                        <p>Lampiran berkas fisik/scan resmi untuk Nomor Surat: {{ $suratMasuk->nomor_surat }}</p>
                    </div>

                    <div class="attachment-body">
                        <img src="{{ asset('storage/' . $filePath) }}" alt="Lampiran Surat Masuk" class="attachment-img">
                    </div>
                </div>
                            ...
                            (function(num) {
                                pdf.getPage(num).then(function(page) {
                                    ...
                                    pageDiv.innerHTML = `
                                        <div>
                                            <div class="attachment-header">
                                                <h3>LAMPIRAN DOKUMEN SURAT MASUK ${pdf.numPages > 1 ? `(Halaman ${num} dari ${pdf.numPages})` : ''}</h3>
                                                <p>Lampiran berkas fisik/scan resmi untuk Nomor Surat: {{ $suratMasuk->nomor_surat }}</p>
                                            </div>
