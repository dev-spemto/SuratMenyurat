...
@section('title', 'Daftar Surat Generator')

@section('content')
<div class="row g-4">

    @php
        // 1. Deteksi Kategori Surat Aktif dari Parameter URL (?jenis= atau ?type=)
        $currentJenis = request('jenis', request('type'));

        // Helper untuk memanggil route undangan/pemberitahuan/permohonan secara aman
        $undanganRoute = function($type) {
            if (\Route::has('surat.create-undangan')) {
                return route('surat.create-undangan', ['type' => $type]);
            }
            if (\Route::has('surat.undangan.create')) {
                return route('surat.undangan.create', ['type' => $type]);
            }
            return route('surat.create', ['jenis' => $type]);
        };
        ...
        $configMap = [
            'sppd' => [
                ...
                'btn_route' => route('surat.create', ['jenis' => 'sppd']),
            ...
            'aktif_belajar' => [
                ...
                'btn_route' => route('surat.create', ['jenis' => 'aktif_belajar']),
            ...
            'aktif_mengajar' => [
                ...
                'btn_route' => route('surat.create', ['jenis' => 'aktif_mengajar']),
            ...
            'und' => [
                ...
                'icon' => 'bi-envelope-open-fill text-primary',
                'btn_label' => 'Buat Undangan Baru',
                'btn_route' => $undanganRoute('und'),
                'stat_label' => 'Total Surat Undangan',
                'stat_count' => $totalUndangan ?? 0,
                'stat_color' => 'text-primary',
                'stat_bg' => 'bg-primary text-primary',
                'table_title' => 'Riwayat Surat Undangan',
                'col_header' => 'PERIHAL / ACARA',
            ],
            'pbh' => [
                'title' => 'Surat Pemberitahuan',
                'icon' => 'bi-megaphone-fill text-danger',
                'btn_label' => 'Buat Pemberitahuan Baru',
                'btn_route' => $undanganRoute('pbh'),
                'stat_label' => 'Total Pemberitahuan',
                'stat_count' => $totalPemberitahuan ?? 0,
                'stat_color' => 'text-danger',
                'stat_bg' => 'bg-danger text-danger',
                'table_title' => 'Riwayat Surat Pemberitahuan',
                'col_header' => 'PERIHAL / KETERANGAN',
            ],
            'pmh' => [
                'title' => 'Surat Permohonan',
                'icon' => 'bi-file-earmark-text-fill text-success',
                'btn_label' => 'Buat Permohonan Baru',
                'btn_route' => $undanganRoute('pmh'),
                'stat_label' => 'Total Surat Permohonan',
                'stat_count' => $totalPermohonan ?? 0,
                'stat_color' => 'text-success',
                'stat_bg' => 'bg-success text-success',
                'table_title' => 'Riwayat Surat Permohonan',
                'col_header' => 'PERIHAL / MAKSUD',
            ],
            'custom' => [
                'title' => 'Surat Custom / Lainnya',
                'icon' => 'bi-file-earmark-plus-fill text-secondary',
                'btn_label' => 'Buat Surat Custom',
                'btn_route' => route('surat.create', ['jenis' => 'custom']),
        ...
        // Fallback jika tanpa parameter ?jenis=
    ...
    @if(session('error'))
        <div class="col-12">
            <div class="alert alert-danger alert-dismissible fade show shadow-sm border-0 d-flex align-items-center gap-2 m-0" role="alert">
    ...
    <div class="col-12">
        <div class="d-flex justify-content-between align-items-center bg-white p-3 p-md-4 rounded-3 shadow-sm border">
            ...
            @if($currentJenis)
                <a href="{{ $currentConfig['btn_route'] }}" class="btn btn-primary fw-semibold px-3 py-2 shadow-sm d-inline-flex align-items-center gap-2">
                        ...
                        <tbody class="border-top-0">
                            @forelse($suratList as $index => $surat)
                                @php 
                                    ...
                                    // DETEKSI DATA SAMPLE MASTER SYSTEM
                                    $isMasterSample = $surat->is_master_sample || \Illuminate\Support\Str::startsWith($surat->nomor_surat, '000/');
                                @endphp
                                <tr>
                                    ...
                                    <td class="py-3 px-3">
                                        <div class="fw-semibold text-dark">{{ $p['penerima'] ?? $surat->tujuan_penerima ?? $surat->tujuan ?? '-' }}</div>
                                    </td>
                                    <td class="py-3 px-3">
                                        <span class="text-muted d-block text-truncate" style="max-width: 250px;">
                                            {{ $p['maksud_dinas'] ?? $p['nama_guru'] ?? $p['nama_siswa'] ?? $p['agenda_acara'] ?? $surat->perihal }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="text-center py-3 px-3">
                                        <div class="d-flex justify-content-center gap-1">
                                            ...
                                            @else
                                                <span class="badge bg-secondary bg-opacity-10 text-secondary align-self-center px-2 py-1">Master</span>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
