...
class DatabaseSeeder extends Seeder
{
    ...
    {
        // 1. Jalankan Seeder Utama & Sampel Master System
        $this->call([
            JenisSuratSeeder::class,
            BidangSeeder::class,
            PegawaiSeeder::class,
            SiswaSeeder::class,
            SuratSeeder::class,       // Generator dokumen sampel (SPPD, UND, PBH, PMH, dll)
            SuratSampleSeeder::class, // Sampel master Surat Masuk & Surat Keluar
        ]);
        ...
        // 4. Sample Fallback Ekstra (Aman & Case-Insensitive)
        $jenisPedoman = JenisSurat::whereIn('kode', ['PED', 'ped'])->first() ?? JenisSurat::first();
        $bidang = Bidang::first();

        if ($jenisPedoman && $bidang) {
            // Sample Fallback Surat Keluar
            $modelSuratKeluar = class_exists(SuratKeluar::class) ? SuratKeluar::class : Surat::class;
            
            $modelSuratKeluar::firstOrCreate(
                [
                    'nomor_urut' => 0,
                    'tahun'      => 2000,
                ],
                [
                    ...
                    'jenis_surat'    => 'biasa',
                    'bidang_id'      => $bidang->id,
                    'nomor_surat'    => sprintf('000/%s/IV.4.AU/%s/I/2000', strtoupper($jenisPedoman->kode), $bidang->kode),
            ...
            // Sample Fallback Surat Masuk (Lengkap dengan nomor_urut & tahun)
